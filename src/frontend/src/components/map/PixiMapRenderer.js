...
export default function PixiMapRenderer({ map, features, heatmaps, onEnterChildMap, onHoverFeature }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            dynamicLayer.zIndex = 2;
            const effectLayer = new Container();
            effectLayer.zIndex = 3;
            const heatmapLayer = new Container();
            heatmapLayer.zIndex = 4;
    ...
    useEffect(() => {
        ...
        if (!vp)
            return;
        const featureLayer = vp.children.find((c) => c.zIndex === 1);
        if (!featureLayer)
            return;
    ...
    return (_jsxs("div", { className: "pixi-renderer-wrap", children: [_jsx("div", { className: "pixi-canvas-host", ref: hostRef }), _jsxs("div", { className: "pixi-toolbar", children: [_jsx("button", { className: "small", onClick: () => setShowLabels((v) => !v), children: showLabels ? '🏷 隐藏标注' : '🏷 显示标注' }), _jsx("button", { className: "small", onClick: resetView, children: "\u2295 \u91CD\u7F6E\u89C6\u56FE" }), _jsxs("span", { className: "muted small", children: ["\u7F29\u653E ", (zoom * 100).toFixed(0), "%"] })] })] }));
}
